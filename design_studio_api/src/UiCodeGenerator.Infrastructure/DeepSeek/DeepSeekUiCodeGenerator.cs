...
public sealed class DeepSeekUiCodeGenerator(
    ...
    ILogger<DeepSeekUiCodeGenerator> logger)
    : IAiUiCodeGenerator
{
    private static readonly JsonSerializerOptions DeepSeekJsonOptions =
    ...
    private static readonly JsonSerializerOptions ModelPayloadJsonOptions =
        new(JsonSerializerDefaults.Web)
        {
            PropertyNameCaseInsensitive = true,
    ...
    public async Task<AiUiGenerationPayload> GenerateAsync(
        GenerateUiRequest request,
        CancellationToken cancellationToken)
    {
        ...
        if (string.IsNullOrWhiteSpace(currentOptions.ApiKey))
            throw new AiConfigurationException(
                "DeepSeek:ApiKey is not configured. Set it with user secrets or the DeepSeek__ApiKey environment variable.");

        using var httpRequest =
            new HttpRequestMessage(HttpMethod.Post, "chat/completions");

        httpRequest.Headers.Authorization =
            new AuthenticationHeaderValue("Bearer", currentOptions.ApiKey);

        httpRequest.Content =
            JsonContent.Create(
                CreateChatRequest(request, currentOptions),
        ...
        if (string.IsNullOrWhiteSpace(content))
            throw new AiProviderException(
                "DeepSeek returned an empty chat completion.",
        ...
        logger.LogInformation(
            "Generated UI code with DeepSeek model {Model}.",
            currentOptions.Model);

        return payload;
    }

    private static DeepSeekChatRequest CreateChatRequest(
        GenerateUiRequest request,
    ...
    private static DeepSeekChatResponse DeserializeDeepSeekResponse(
        string responseText)
    {
        try
        {
            return JsonSerializer.Deserialize<DeepSeekChatResponse>(
                       responseText,
                       DeepSeekJsonOptions)
                   ?? throw new AiProviderException(
    ...
    private static AiUiGenerationPayload DeserializeModelPayload(
        string content)
    {
        try
        {
            return JsonSerializer.Deserialize<AiUiGenerationPayload>(
                       content,
                       ModelPayloadJsonOptions)
                   ?? throw new AiProviderException(
                       "DeepSeek returned empty UI generation JSON.");
        }
        catch (JsonException exception)
        {
            throw new AiProviderException(
                "DeepSeek returned invalid UI generation JSON.",
                innerException: exception);
        }
    }

    private static void NormalizeAndValidatePayload(
        AiUiGenerationPayload payload,
        GenerateUiRequest request)
    {
        payload.Preview ??= new();
        ...
        if (string.IsNullOrWhiteSpace(payload.SourceCode.Framework))
            payload.SourceCode.Framework =
                request.Framework.Trim().ToLowerInvariant();

        if (string.IsNullOrWhiteSpace(payload.SourceCode.Language))
            payload.SourceCode.Language =
                request.Framework.Equals(
                    "react",
                    StringComparison.OrdinalIgnoreCase)
                    ? "tsx"
                    : "html";

        if (payload.SourceCode.Files.Count == 0 ||
            payload.SourceCode.Files.Any(
                file => string.IsNullOrWhiteSpace(file.Content)))
                    throw new AiProviderException(
                        "DeepSeek did not return any usable source code files.");
    }

    private static void ThrowProviderException(
        ...
        string responseText)
    {
        ...
        var providerError =
            TryReadProviderError(responseText);

        var message =
            providerError?.Message
            ?? $"DeepSeek API returned HTTP {providerStatusCode}.";
    ...
    private static DeepSeekError? TryReadProviderError(
        string responseText)
    {
        ...
        try
        {
            return JsonSerializer.Deserialize<DeepSeekErrorEnvelope>(
                responseText,
                DeepSeekJsonOptions)?.Error;
        }
        catch (JsonException)
        {
            return null;
